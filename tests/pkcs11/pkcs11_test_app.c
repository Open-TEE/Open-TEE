...
static void sign_and_verify(CK_SESSION_HANDLE session, CK_MECHANISM *mechanism)
{
	...
	char sig[512]; /* Avoiding malloc and reserving enough big buffer for all
			  signatures */
	char *mech_type;
	CK_RV ret;
...
static void hash(CK_SESSION_HANDLE session, CK_MECHANISM *mechanism)
{
	...
	char hash[biggest_hash_len]; /* Avoiding malloc. Reserving enough big buffer
					for all hash */
	char *mech_type;
	CK_RV ret;
...
static int initialize_token(CK_SLOT_ID slot_id)
{
	...
	/* see if we have a label assigned to the token, if so then no need to
	 * initialize it */
	...
	/* Next open a session and login as the SO user, to set the normal user
	 * passowrd */
