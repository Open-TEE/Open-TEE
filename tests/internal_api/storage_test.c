...
static uint8_t modulus[] = "\x88\x3e\x6b\x47\x4e\xdb\x4a\x36\xa4\xf4\x7f\x2f"
			   "\x88\xc9\xdd\x96\x45\xf0\xee"
			   "\x9e\x1d\xe9\xf5\xc4\x1d\x36\x2e\xbf\x1e\x61\x25"
			   "\x52\x62\x62\x6e\xce\x95\xc8"
			   "\xcb\xd7\x0e\x34\x37\xe1\x77\xf1\x32\x20\x2d\xa6"
			   "\x1e\x6b\x4f\x9f\x94\x7d\xc9"
			   "\x8e\xad\x78\x7e\xae\x75\xac\xed\x10\x7f\xca\xd2"
			   "\x2f\xab\x79\x37\x8d\xc1\xeb"
			   "\xec\x8b\xcb\x6a\x1d\x1b\x9a\xd8\x13\xd4\x6a\x4b"
			   "\x65\x67\xa7\xa9\xae\x9c\x47"
			   "\xb0\xfc\xc4\x46\xb9\xbc\x44\x6a\x1a\xb0\x29\x08"
			   "\xc7\x3b\x0a\xd2\xea\x01\x40"
			   "\x22\x24\x97\xd7\x80\xe0\x31\x89\xb6\x0b\x51\xef"
			   "\x4c\x79\xb5\xfb\x51\x8c\xb2"
			   "\x18\x49\xfe\x3c\xd6\x1a\x07\x20\x4e\xaa\x5a\xab"
			   "\x80\x2e\xa9\x0c\x67\x00\xb4"
			   "\xaa\xa1\xce\xbb\xc6\xb1\x57\x02\x0a\xfd\x65\xe3"
			   "\xf8\x05\x1b\xf8\xec\x6c\x3d"
			   "\xe2\x1c\x24\xf7\x9b\xf9\x82\x42\xe5\x91\x50\xf4"
			   "\x21\x37\xce\x0f\xde\x85\x52"
			   "\x13\x6d";
...
static uint32_t gen_rsa_key_pair_and_save_read()
{
	...
	size_t objID_len = 64;
	uint32_t flags =
	    TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_OVERWRITE;
	void *data;
	size_t data_len = 12;
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags,
					 handler, data, data_len, (TEE_ObjectHandle *)NULL);
	if (ret != TEE_SUCCESS) {
...
static uint32_t rename_per_obj_and_enum_and_open_renameObj()
{
	...
	uint32_t flags =
	    TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_OVERWRITE;
	TEE_ObjectEnumHandle iter_enum = (TEE_ObjectEnumHandle)NULL;
	TEE_ObjectInfo info;

	char original_obj_id[TEE_OBJECT_ID_MAX_LEN], obj_id[TEE_OBJECT_ID_MAX_LEN],
	    new_obj_id[TEE_OBJECT_ID_MAX_LEN];
	size_t original_obj_id_len = 15, obj_id_len = TEE_OBJECT_ID_MAX_LEN, new_obj_id_len = 20,
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)original_obj_id,
					 original_obj_id_len, flags, (TEE_ObjectHandle)NULL,
	...
	if (ret != TEE_ERROR_ACCESS_CONFLICT) {
		PRI_FAIL("Object rename should have failed with TEE_ERROR_ACCESS_CONFLICT "
			 ": 0x%x",
			 ret);
		goto err;
	}
...
static uint32_t data_stream_write_read()
{
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags, key,
					 NULL, 0, &per_han);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Create persisten object failed : 0x%x", ret);
		goto err;
	...
	if (count != write_data_len) {
		PRI_FAIL("Read data length mismatch (read[%lu]; expected[%lu])", count,
			 write_data_len);
		goto err;
	}
...
static uint32_t object_data_size()
{
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags,
					 transObj, data, data_len, (TEE_ObjectHandle *)NULL);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Create persisten object failed : 0x%x", ret);
		goto err;
	}
...
static uint32_t persisten_object_write_and_read()
{
	...
	ret =
	    TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags, &object);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Open failed : 0x%x", ret);
		goto err;
	}
...
static uint32_t persistent_initial_data(bool fromStorage)
{
	...
	size_t objID_len = 64;
	uint32_t flags =
	    TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_OVERWRITE | TEE_DATA_FLAG_ACCESS_READ;
	...
	if (fromStorage) {
		ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, 0,
						 NULL, (void *)init_data, init_data_len,
		...
		ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len,
						 flags, (TEE_ObjectHandle)NULL, init_data,
						 init_data_len, &object);
		if (ret != TEE_SUCCESS) {
			PRI_FAIL("Create persisten object failed : 0x%x", ret);
			goto err;
		}
	...
	if (info.dataPosition != init_data_len) {
		PRI_FAIL("Object position wrong (expectedPos[%lu]; "
			 "info.dataPosition[%u])",
			 init_data_len, info.dataPosition);
		goto err;
	}
...
static uint32_t persisten_object_init_data()
{
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags,
					 (TEE_ObjectHandle)NULL, write_data, write_data_len,
					 (TEE_ObjectHandle *)NULL);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Create persisten object failed : 0x%x", ret);
		goto err;
...
static uint32_t overwrite_persisten()
{
	...
	size_t objID_len = 64;
	uint32_t flags = TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_ACCESS_READ |
	...
	ret = TEE_CreatePersistentObject(
	    TEE_STORAGE_PRIVATE, (void *)objID, objID_len, TEE_DATA_FLAG_ACCESS_WRITE_META,
	    (TEE_ObjectHandle)NULL, (void *)&counter, sizeof(counter), (TEE_ObjectHandle *)NULL);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Create persisten object failed : 0x%x", ret);
		goto err;
	}
...
static uint32_t do_data_stream_write(bool fromStorage, bool addAesAttr)
{
	...
	size_t objID_len = 64;
	uint32_t flags =
	    TEE_DATA_FLAG_ACCESS_WRITE_META | TEE_DATA_FLAG_OVERWRITE | TEE_DATA_FLAG_ACCESS_WRITE;
	char data[50] = {0};
	size_t data_len = 27;
	uint32_t fn_ret = 1; // Initialized error return
	TEE_ObjectInfo info;
	...
	if (fromStorage) {
		ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, 0,
						 transObj, NULL, 0, (TEE_ObjectHandle *)NULL);
		if (ret != TEE_SUCCESS) {
			PRI_FAIL("Create persisten object failed : 0x%x", ret);
			goto err;
		}
		...
		ret =
		    TEE_OpenPersistentObject(TEE_STORAGE_PRIVATE, objID, objID_len, flags, &object);
		if (ret != TEE_SUCCESS) {
			PRI_FAIL("Open failed : 0x%x", ret);
			goto err;
		}
...
static uint32_t do_data_position(bool fromStorage)
{
	...
	if (fromStorage) {
		ret =
		    TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, 0,
					       transObj, data, data_len, (TEE_ObjectHandle *)NULL);
		if (ret != TEE_SUCCESS) {
			PRI_FAIL("Create persisten object failed : 0x%x", ret);
			goto err;
		...
		ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len,
						 flags, transObj, data, data_len, &object);
		if (ret != TEE_SUCCESS) {
			PRI_FAIL("Create persisten object failed : 0x%x", ret);
			goto err;
		}
...
static uint32_t truncate_per_object()
{
	...
	size_t read_bytes = 0, read_buf_s = 50, fn_ret = 1; // Initialized error
							    // return
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, (void *)objID, objID_len, flags, NULL,
					 init_data, init_data_len, &perObj);
	if (ret != TEE_SUCCESS) {
		PRI_FAIL("Create persisten object failed : 0x%x", ret);
		goto err;
	}
...
static uint32_t enumerate_per_objecs()
{
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, objID_1, objID_1_len, flags, NULL,
					 NULL, 0, &obj1);
	if (ret != TEE_SUCCESS) {
	...
	for (i = 0; i < 5; ++i) {
		...
		ret = TEE_GetNextPersistentObject(iter_enum, &info, fromStorageObjID,
						  &fromStorageObjIDLen);
		if (ret == TEE_SUCCESS) {
	...
	if (obj1_f && !obj2_f && !obj3_f) {
		PRI_FAIL("Did not found all objects (1) (obj1[%u]; obj2[%u]; obj3[%u])", obj1_f,
			 obj2_f, obj3_f);
		goto err;
	}

	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, objID_2, objID_2_len, flags, NULL,
					 NULL, 0, &obj2);
	if (ret != TEE_SUCCESS) {
	...
	ret = TEE_CreatePersistentObject(TEE_STORAGE_PRIVATE, objID_3, objID_3_len, flags, NULL,
					 NULL, 0, &obj3);
	if (ret != TEE_SUCCESS) {
	...
	if (obj1_f || obj2_f || obj3_f) {
		PRI_FAIL("Did not found all objects (2) (obj1[%u]; obj2[%u]; obj3[%u])", obj1_f,
			 obj2_f, obj3_f);
		goto err;
	}
	...
	if (obj1_f || obj3_f) {
		PRI_FAIL("Did not found all objects (3) (obj1[%u]; obj2[%u]; obj3[%u])", obj1_f,
			 obj2_f, obj3_f);
		goto err;
	}
...
static uint32_t get_rsa_buffer_attr()
{
	...
	ret = TEE_GetObjectBufferAttribute(rsa_keypair, TEE_ATTR_RSA_MODULUS, buf, &buf_len);
	if (ret != TEE_ERROR_SHORT_BUFFER) {
		PRI_FAIL("Should have returned TEE_ERROR_SHORT_BUFFER "
			 "(TEE_ATTR_RSA_MODULUS) : 0x%x",
			 ret);
		goto err;
	}
	...
	ret = TEE_GetObjectBufferAttribute(rsa_keypair, TEE_ATTR_DSA_BASE, buf, &buf_len);
	if (ret != TEE_ERROR_ITEM_NOT_FOUND) {
	...
	buf_len = 200;
	ret =
	    TEE_GetObjectBufferAttribute(rsa_keypair, TEE_ATTR_RSA_PRIVATE_EXPONENT, buf, &buf_len);
	if (ret != TEE_SUCCESS) {
...
static uint32_t do_popu_ecc(uint32_t obj_type)
{
	...
	if (obj_type == TEE_TYPE_ECDH_KEYPAIR || obj_type == TEE_TYPE_ECDSA_KEYPAIR) {
		// R
...
static uint32_t do_gen_ecc_key(uint32_t obj_type, uint32_t key_size, uint32_t curve)
{
	TEE_Result ret;
	TEE_ObjectHandle key = NULL;
	...
	ret = TEE_GetObjectBufferAttribute(key, TEE_ATTR_ECC_PRIVATE_VALUE, buffer, &bufferLen);
	if (ret != TEE_SUCCESS) {
...
uint32_t storage_test(uint32_t loop_count)
{
	...
	for (i = 0; i < loop_count; ++i) {
		...
		if (generate_ecc_key() || populate_ecc() || popu_rsa_pub_key() ||
		    enumerate_per_objecs() || get_rsa_buffer_attr() || truncate_per_object() ||
		    persistent_initial_data(true) || persistent_initial_data(false) ||
		    object_data_size() || data_stream_write() || data_stream_write_read() ||
		    multiple_writes() || persisten_object_write_and_read() ||
		    persisten_object_init_data() || data_position() || overwrite_persisten() ||
		    gen_rsa_key_pair_and_save_read() || closeTransientObject() ||
		    closePersistantObject() || rename_per_obj_and_enum_and_open_renameObj()) {
			test_have_fail = 1;
			break;
		}
	}
