...
enum Command { CMD_HELLO, CMD_QUIT, CMD_ENCRYPT, CMD_DECRYPT, CMD_ADDKEY, CMD_REMKEY, CMD_INVALID };
...
void send_msg_browser(std::string out_msg)
{
	...
	std::cout << char(out_msg_len >> 0) << char(out_msg_len >> 8) << char(out_msg_len >> 16)
		  << char(out_msg_len >> 24);
...
CK_OBJECT_HANDLE get_key_handle(std::string keyid, CK_KEY_TYPE keytype, CK_OBJECT_CLASS obj_class,
...
uint32_t encrypt(CK_SESSION_HANDLE *session, Document *json)
{
	...
	// yes we are
	// silence
	hKey = g_key; // get_key_handle((*json)["key"].GetString(), CKK_AES,
		      // CKO_SECRET_KEY, session);
	...
	for (i = 0; i < payload.length();) {
		...
		ret = func_list->C_EncryptUpdate(*session, (CK_BYTE_PTR)clear, clear_len,
						 (CK_BYTE_PTR)cipher + i - j, &cipherpart_len);
		if (ret) {
...
uint32_t decrypt(CK_SESSION_HANDLE *session, Document *json)
{
	...
	payload = base64_decode((*json)["payload"].GetString());
	std::cerr << "payload_decoded: " << payload << "payloadlen " << payload.length()
		  << std::endl;
	...
	for (i = 0; i < payload.length();) {
		...
		ret = func_list->C_DecryptUpdate(*session, (CK_BYTE_PTR)encrypted, encrypted_len,
						 (CK_BYTE_PTR)decrypted + i - j, &cipherpart_len);
		if (ret) {
...
uint32_t cmd_handler(CK_SESSION_HANDLE *session, Document *json)
{
	...
	//- in this early case the reply is just a simple text field accompanied with
	// a msg
