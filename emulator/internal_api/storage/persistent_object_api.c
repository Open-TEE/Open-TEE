				    ...
				    uint32_t flags, TEE_ObjectHandle *object)
{
	...
	if (object == NULL || objectID == NULL || objectIDLen > TEE_OBJECT_ID_MAX_LEN) {
		OT_LOG_ERR("TEE_OpenPersistentObject: Object null OR objectID null OR objectIDLen "
			   "too big\n");
	...
	if (storageID != TEE_STORAGE_PRIVATE) {
		OT_LOG_ERR(
		    "TEE_OpenPersistentObject: Only supported storageID is TEE_STORAGE_PRIVATE\n");
				      ...
				      TEE_ObjectHandle *object)
{
	...
	if (objectID == NULL) {
		...
		OT_LOG_ERR("TEE_CreatePersistentObject panicking due attributes object is type "
			   "TEE_TYPE_DATA\n");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
