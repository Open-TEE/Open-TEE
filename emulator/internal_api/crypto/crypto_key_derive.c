		   ...
		   TEE_ObjectHandle derivedKey)
{
	...
	TEE_Attribute *derivedKey_x, *derivedKey_y, *generic_sec_attr, *operation_d;
	...
	if (!operation) {
		...
		OT_LOG_ERR(
		    "TEE_DeriveKey panics due operation class not TEE_OPERATION_KEY_DERIVATION");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
		...
		OT_LOG_ERR(
		    "TEE_DeriveKey panics due derivedKey object type not TEE_TYPE_GENERIC_SECRET");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	...
	operation_d =
	    get_attr_from_attrArr(TEE_ATTR_ECC_PRIVATE_VALUE, operation->key_data->gp_attrs.attrs,
				  operation->key_data->gp_attrs.attrs_count);
	generic_sec_attr =
	    get_attr_from_attrArr(TEE_ATTR_SECRET_VALUE, derivedKey->key->gp_attrs.attrs,
	...
	rv_mbedtls = mbedtls_mpi_read_binary(&Q.private_X, derivedKey_x->content.ref.buffer,
					     derivedKey_x->content.ref.length);
	if (rv_mbedtls != 0) {
	...
	rv_mbedtls = mbedtls_mpi_read_binary(&Q.private_Y, derivedKey_y->content.ref.buffer,
					     derivedKey_y->content.ref.length);
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
	...
	rv_mbedtls = mbedtls_mpi_read_binary(&d, operation_d->content.ref.buffer,
					     operation_d->content.ref.length);
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
	...
	rv_mbedtls = mbedtls_ecdh_compute_shared(operation->ctx.ecc.grp, &z, &Q, &d,
	...
	if (mbedtls_mpi_write_binary(&z, generic_sec_attr->content.ref.buffer,
				     generic_sec_attr->content.ref.length)) {
