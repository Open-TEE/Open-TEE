...
/* This function is collection algorithms/key sizes that is
...
TEE_Result valid_key_and_operation(TEE_ObjectHandle key, TEE_OperationHandle operation)
{
	...
	if (key->objectInfo.maxObjectSize > operation->operation_info.maxKeySize) {
		OT_LOG_ERR("OperationKeyError: Object key maxKeySize too big (key[%u]; "
			   "operation[%u]",
	...
	if (!object_type_compatible_to_op(key->objectInfo.objectType,
		...
		OT_LOG_ERR("OperationKeyError: Key object type conflicts with operation "
			   "mode (keyObjectType[%u]; opeationMode[%u])",
	...
	if (valid_key_type_for_operation_algorithm(key->objectInfo.objectType,
		...
		OT_LOG_ERR("OperationKeyError: Key object type conflict with operation "
			   "algorithm (keyObjectType[%u]; opeationAlg[%u])",
	...
	if (key_usage_allow_operation(key->objectInfo.objectUsage, operation->operation_info.mode,
				      operation->operation_info.algorithm)) {
		OT_LOG_ERR("OperationKeyError: Key object usage conflict with operation "
			   "mode or/and operation algorithm (keyObjectUsage[%u]; "
			   "opeationMode[%u]; opeationAlg[%u])",
