...
/* Note: General operation functions from the Cryptographic operations API found
 * in this file */
...
TEE_Result TEE_SetOperationKey(TEE_OperationHandle operation, TEE_ObjectHandle key)
{
	...
	if (operation->operation_info.algorithm == TEE_ALG_AES_XTS) {
		OT_LOG_ERR("TEE_SetOperationKey panicking due operation algorithm "
			   "TEE_ALG_AES_XTS");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
	...
	if (ret != TEE_SUCCESS) {
		OT_LOG_ERR("TEE_SetOperationKey panicking (search \"OperationKeyError\" "
			   "from syslog)");
