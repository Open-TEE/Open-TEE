...
static void __attribute__((constructor)) storage_init() { INIT_LIST(&storage_element_head); }
	...
	if (object_meta_info.attribute_size > 0) {
		...
		*attrs = calloc(1, object_meta_info.attribute_size);
		...
		if (object_meta_info.attribute_size !=
		    ext_read_stream(storage_blob_id, object_meta_info.attr_begin, *attrs,
				    object_meta_info.attribute_size)) {
			OT_LOG(LOG_ERR, "Cannot read object meta data\n");
			ret_val = TEE_ERROR_GENERIC;
			goto err_2;
		}
					  ...
					  size_t initialDataLen)
{
	...
	if (object_meta_info.attribute_size > 0) {
		if (object_meta_info.attribute_size !=
		    ext_write_stream(ss_id, object_meta_info.attr_begin, attrs,
				     object_meta_info.attribute_size)) {
			goto err;
		}
	}
	...
	// write initi data
	if (initialData) {
		if (initialDataLen != ext_write_stream(ss_id, object_meta_info.data_begin,
					  ...
					  size_t newObjectIDLen)
{
	...
	if (!ext_change_object_ID(ss_id, objectID, objectIDLen, newObjectID, newObjectIDLen)) {
				  ...
				  uint32_t *count, size_t *pos)
{
	...
	*count = ext_read_stream(storage_blob_id, object_meta_info.data_begin + *pos, buffer, size);
				   ...
				   size_t *pos)
{
	...
	write_bytes =
	    ext_write_stream(storage_blob_id, object_meta_info.data_begin + *pos, buffer, size);
	if (write_bytes != size) {
...
TEE_Result MGR_TEE_TruncateObjectData(void *objectID, size_t objectIDLen, size_t size, size_t *pos)
{
	...
	rv = ext_truncate_storage_blob(storage_blob_id, object_meta_info.data_begin + size);
	if (rv != TEE_SUCCESS) {
