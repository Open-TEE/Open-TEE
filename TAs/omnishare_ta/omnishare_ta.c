...
/* GP framework is defining session context parameter for opensession, invokecmd
 * and closesession function. This context is allocated and placed into this
 * parameter. */
struct session_ctx {
...
/* Omnishare TEE spesific RSA key is generated once and only once at create
 * entry point function. RSA key is saved into secure storage (ss). */
...
/* Corresponding IV vector. For simplicity sake, the IV vector in every AES
 * operation is kept as a zero and this with AES CTR mode is very very unsecure,
 * Very bad. */
 ...
 * Wraping an AES operation. Function will allocated operation and setting a
 * key. Then it will init a cipher operation and encrypt or decrypt data.
				     ...
				     void *out_data, uint32_t *out_data_len)
{
	...
	/* Omnishare TA is supporting only small files and therefore there is no need
	 * for "pipeline" style encrypting or decrypting. The do final can do it in
	 * one go TEE_CipherDoFinal: TEE Core API p-157 */
 ...
 * \param aes_key_size If buffer is not NULL, aes aes_key buffer size. Size is
 * updated.
 * \param aes_key_object If parameter is not NULL, new aes key object is
 * allocated with an aes key. Note: aes_key_object should be freed after usage.
 * \return GP return values.
 */
				     ...
				     TEE_ObjectHandle *aes_key_object) /* Opaque handle */
{
	...
	if (aes_key) {
		...
		/* Getting the key from GP object. It is getting the key from AES transient
		 * object that we allocated at the beginning of function. No extra step for
		 * getting key, because object usage is not restricted by default
	...
	/* If caller want to have a handle to newly created key. If not, free newly
	 * created object*/
	if (aes_key_object)
...
/*!
 * \brief get_dir_key
 * Function is decrypting omnishare key chain. Key chain the first key is cached
 * AES (provided in open session function)
 * \param paramTypes
 * \param params
			      ...
			      TEE_ObjectHandle *dir_key) /* Opaque handle */
{
	...
	/* Function is returning a handle to direcotry key and it will be allocated
	 * now. All directory keys are AES keys. TEE_AllocateTransientObject: TEE Core
	 * API p-102 */
	...
	/* Key chain first key is cached AES key, which were provided at open session
	 * function. Copiying the first AES key into object handle
	...
	/* Something is passed at index zero. If it is a key chain, it should be
	 * buffer parameter and it should be as an input parameter */
	...
	for (i = 0; i < key_chain->key_count; i++) {
		...
		/* We have more keys than one. Lets read next key from key chain and decrypt
		 * it */
 ...
 * Creates an AES key and encrypt it with directory key. This function is called
					   ...
					   TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	...
	/* First of all it need to be check if the output buffer at index three is big
	 * enough If is not big enough, return TEE_ERROR_SHORT_BUFFER error code and
	 * required size */
 ...
 * Creates a file key and using it for encrypting the file. The file key is
 * placed to beginning of the file
 * \param dir_key
 * \param params
 * \return
 */
					 ...
					 TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	...
	/* If output buffer is big enough to hold the encrypted key and encrypted
	 * data. encrypted data is same size as a plain data. If buffer is not big
	 * enough, return TEE_ERROR_SHORT_BUFFER and required size */
	...
	/* Reduce the ouput buffer size, because we just wrote the key into output
	 * buffer */
	...
	/* Output buffer size is holding the "encrypted" plain data size, but this
	 * buffer is containing the file key at the beginning of file and therefore we
	 * are adding also encrypted key size into this */
	params[3].memref.size += write_bytes;
out:
	/* TEE_FreeTransientObject: TEE Core API p-105 */
 ...
 * Reading AES key from the beginning of file and using it for decrypting the
 * file.
 * \param dir_key
 * \param params
 * \return
					 ...
					 TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	...
	/* If output buffer is big enough. If not, return TEE_ERROR_SHORT_BUFFER and
	 * required size*/
	...
	/* We need to use fresly decrypted key and therefore we are needing a object
	 * handle, which can be populated with decrypted key.
	 * TEE_AllocateTransientObject: TEE Core API p-102 */
	...
	/* Populating allocated object. TEE_PopulateTransientObject: TEE Core API
	 * p-107 */
...
/*!
 * \brief do_crypto
 * Omnishare crypto operations. Function is containg a common functionality of
 * crypto operations and crypto operation parser, which will be calling spesific
 * crypto operation. Note: Crypto operations are not saving anything into secure
 * storage.
 * \param paramTypes
 * \param params
 * \return GP return values.
			    ...
			    TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	...
	/* Crypto operation have commons following parameters. Checking parameters one
	 * by one for purpose of printing debug message
	 *
	...
	/* Do spesific crypto operation. Operation type is at index 1 and it is in
	 * value A member */
 ...
 * Creates an AES key. This key will be the first key of key chains. It will be
 * encrypted with omnishare spesific RSA key.
 * \param paramTypes
 * \param params
 * \return GP return values
 */
				  ...
				  TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	 ...
	 * If is not big enough, return TEE_ERROR_SHORT_BUFFER error code and required
	 * size */
	...
	/* Encrypt fresly created AES key with RSA key and place result into output
	 * buffer */
 ...
 * Function is using omnishare spesific RSA key for decrypting the data blob.
 * Data blob is containing AES key (the first key of the key chains == root key)
 * \param params
 * \return GP return values
 */
static TEE_Result set_oms_aes_key(TEE_Param *params) /* TEE_Param: TEE Core API p-36 */
{
	...
	/* Next we are initing the first key of keychain. It is previously decrypted
	 * AES key */
	...
	/* If everything went fine, now we have open to global object handle for ready
	 * to use. One is a RSA key and one is AES key */
...
TEE_Result TA_EXPORT TA_CreateEntryPoint(void)
{
	...
	/* Create entry point is trying to find the omnishare spesific RSA key and
	 * open it. If key is not found, the key is created and saved into secure
	 * storage */

	/* Using open persisten object for determing if RSA key exist. We also could
	 * be using persistent storage enumerator for this task.
	...
	if (tee_rv == TEE_SUCCESS) {
		...
		/* OK: It just is that the object is not found and therefore it need to be
		 * created*/
	} else {
	...
	/* Creating a transient object for the new RSA key. Transient object is
	 * needed, because generate key is accepting only transient objects
	 * TEE_AllocateTransientObject: TEE Core API p-102 */
...
out:
	/* Transient object is not any more needed. Lets free it and not leak the
	 * memory TEE_FreeTransientObject: TEE Core API p-105 */
...
{
	/* TA is going to be destroyed. It is a good practice close all our resources
	 * eg. persistent object. We have oms_RSA_keypair_object open and it need to
	 * be closed TEE_CloseObject: TEE Core API p-101 */
					      ...
					      void **sessionContext)
{
	...
	/* Session context is controlled by TEE framework. It will be passed as a
	 * function parameter in invokeCMD and closeSession functions. Omnishare is
	 * using this parameter for determing, which session is invoking command or
	 * closing session. */
	...
	/* Using paramTypes for determing the open session type (create root key or
	 * crypto operation). This is one way of doing this, because in this case we
	 * have only two possible options. Another way is just use one of the
	 * parameters for determining open session type eg paramType 3 is value
	 * parameter */
	if (TEE_PARAM_TYPE_GET(paramTypes, 0) == TEE_PARAM_TYPE_NONE &&
	...
	/* Session is not opened if return value going to be something else than
	 * TEE_SUCCESS and in that case we need all memory that we are allocated in
...
void TA_EXPORT TA_CloseSessionEntryPoint(void *sessionContext)
{
	...
	/* Session context need to be freed, because this session is not any more are
	 * contacting TEE. Because destroy entrypoint is Void function, this is a last
	 * place where session context can be freed or else we would leak a memory
	 * TEE_Free: TEE Core API p-86 */
