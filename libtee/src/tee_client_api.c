...
/* Operation is started when message is send out and stopped when response
 * message is received */
...
/* Only one connection to TEE is allowed! Therefore only one context internal
 * can be init */
#define CTX_INTERNAL_INIT 0xFB83DA36

struct shared_mem_internal {
	char shm_uuid[SHM_MEM_NAME_LEN]; /*!< the shared memory object that has been
					    created */
 ...
 * Create a memory mapped shared memory object that can be used to transfer data
 * between the TEE and the Client application
...
static TEEC_Result get_shm_from_manager_and_map_region(struct shared_mem_internal *shm_internal)
{
	...
	/* mmap does not allow for the size to be zero, however the TEEC API allows
	 * it, so map a size of 1 byte, though it will probably be mapped to a page */
				     ...
				     enum mem_type type)
{
	...
	/* If we are allocating memory the buffer is the new mmap'd region, where as
	 * if we are only registering memory the buffer has already been alocated
	 * locally, so the mmap'd region is where we will copy the data just before we
	 * call a command in the TEE, so it must be stored seperatly in the
	 * "implementation deined section" */
	if (type == ALLOCATED)
 ...
 * Convert the TEE operation into a generic format do that it can be sent to the
 * TA
 ...
 * When the response message comes from the TA we must copy the data back into
...
static TEEC_Result register_temp_refs(TEEC_Operation *operation, TEEC_SharedMemory *temp_shm)
{
	...
	/* Context is not used. We have only on context, which is initialized during
	 * the TEEC_InitializeContext() function. Therefore we can have a context here
	 *
	 * Note: This need to be changed, if we are allowing to have more than one
	 * opentee-process*/
...
TEEC_Result TEEC_InitializeContext(const char *name, TEEC_Context *context)
{
	...
	/* We ignore the name as we are only communicating with a single instance of
	 * the emulator */
	(void)name;
...
void TEEC_FinalizeContext(TEEC_Context *context)
{
	...
	/* We are not actually receiving any data from TEE. This call is here for
	 * blocking purpose. It is preventing closing this side socket before TEE
	 * closes connection. With this it is easier segregate expected disconnection
	 * and not expected disconnection. This blocking will end when TEE closes its
	 * side socket. */
			     ...
			     uint32_t *return_origin)
{
	...
	if (com_ret == -1) {
		 ...
		 * incomming. Error or Response to open session message. Worst case
		 * situation is that task is complited, but message delivery only failed.
		 * Just report communication error and dump problem "upper layer". */
		goto err_com_2;
	}
			       ...
			       TEEC_Operation *operation, uint32_t *return_origin)
{
	...
	if (com_ret == -1) {
		...
		OT_LOG(LOG_ERR, "Received bad message, discarding");
		/* TODO: Do what? End session? Problem: We do not know what message was
		 * incomming. Error or Response to invoke cmd message. Worst case situation
		 * is that task is complited, but message delivery only failed. For now,
		 * just report communication error and dump problem "upper layer". */
		goto err_com_1;
	}
...
void TEEC_RequestCancellation(TEEC_Operation *operation)
{
	...
	/* Set operation to be canceled. What is signaling operation cancelation is
	 * opeartion imp-member. If imp NULL, operation is cancelled. */
	operation->imp = NULL;

	/* Operation may have send already to TEE. If started member NULL, operation
	 * is not send to TEE and is queued in CA */
