 ...
 * \param key [IN/OUT] A non-NULL buffer to return the wrapped device specific
 * key
 * \param key_size [IN/OUT] the size of the buffer as input, and the actual size
 * on output
 * \return 0 on success
 */
 ...
 * Initialize the Trusted application with the root key of the omnishare
 * hirarachy. The root key is encrypted with the device specific RSA Key before
 * it is stored to the root directory of the omnishare tree.
 ...
 * \param key_chain [IN] The chain of keys tha tlead from the root dir down to
 * the level where the current file operations are taking place.
 ...
 * \param dest_len [IN/OUT] Length of the buffer available for output, on out
 * the actual length used
 * \return 0 on success
 */
