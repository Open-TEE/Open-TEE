...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "generate") == 0) {
        ...
        command = CMD_ENCRYPT_HASH;
    } else if (strcmp(argv[1], "dv") == 0) {
        command = CMD_DECRYPT_VERIFY;
    } else if (strcmp(argv[1], "encryptaesgcm") == 0) {
        command = CMD_ENCRYPT_AES_GCM;
    } else if (strcmp(argv[1], "decryptaesgcm") == 0) {
        command = CMD_DECRYPT_AES_GCM;
    } else {
        printf("Choose a valid command: generate, retrieve, encrypt, decrypt, eh (encryption-hash), dv (decryption-verification), encryptaesgcm, decryptaesgcm\n");
        exit(1);
    }
    ...
    if (command == CMD_GENERATE_AES_KEY) {
        ...
        */
    ...
    } else if (command == CMD_ENCRYPT_HASH || command == CMD_DECRYPT_VERIFY) {

        if (argc < 5) {
            fprintf(stderr, "Usage: %s [eh/dv] <key id 1> <key id 1> <input file> <output file>\n", argv[0]);
            exit(1);
        }
        ...
        // Output size should be input size + size of hash 
        size_t hash_size = 32; // Size of SHA-256 hash
        size_t required_size = fsize + hash_size; // because size after encryption size will still be fsize 
        if (command == CMD_DECRYPT_VERIFY) {
            required_size = fsize - hash_size;
        } 
        ...
        // Prepare the operation again with the correct buffer size (to recieve the ciphertext and hash)
        op.params[2].tmpref.buffer = output_data;
        op.params[2].tmpref.size = required_size;
    ...
    } else if (command == CMD_ENCRYPT_AES_GCM || command == CMD_DECRYPT_AES_GCM) {

        if (argc < 5) {
            fprintf(stderr, "Usage: %s [encryptaesgcm/decryptaesgcm] <key id > <input file> <output file>\n", argv[0]);
            exit(1);
        }
        ...
        size_t iv_size = 12;
        size_t tag_size = 16;
        size_t required_size = fsize + iv_size + tag_size;
        if (command == CMD_DECRYPT_AES_GCM) {
            required_size = fsize - iv_size - tag_size;
        } 
